...
# Константы для шагов заполнения заявки
...
def get_keyboard(buttons_list):
    ...
    for button_text in buttons_list:
        label = button_text[:39] # Ограничение, чтобы избежать конфликта с VK API
...
def start_ticket_process(user_vk_id, write_msg_func):
    try:
        ...
        write_msg_func(user_vk_id, "Произошла ошибка при загрузке категорий. Ппожалуйста, попробуйте позже")
        return None

def process_ticket_step(user_vk_id, message_text, user_ticket_data, user_ticket_step, write_msg_func, authorized_admins=None):
    """Обрабатывает шаги создания заявки.Словарь authorized_users из главного цикла для отправки уведомлений"""
    ...
    if current_step == STEP_CATEGORY:
        try:
            ...
            if result:
                ...
                write_msg_func(user_vk_id, "Категория не найдена. Пожалуйста, попробуйте снова")
                return STEP_CATEGORY
        except Exception as e:
            print("Ошибка при обработке категории:", e)
            write_msg_func(user_vk_id, "Произошла ошибка. Пожалуйста, попробуйте снова")
            return STEP_CATEGORY
        ...
        write_msg_func(user_vk_id, "Пожалуйста, кратко и своими словами опишите возникшую проблему")
        ...
        try:
            ...
            if new_ticket_id:
                # Список всех администраторов из БД
                cur.execute("SELECT user_id, full_name FROM users WHERE role_id = 2") 
