...
def list_active_tickets(user_id):
    ...
    try:
        ...
        # Выборка заявок из бд (исключаются заявки со статусом 'Выполнена', 'Отклонена')
        cur.execute("""
        ...
        for row in rows:
            ...
            # Формирование строки вида "Заявка №(номер) (Приоритет)"
...
def change_ticket_status(user_id, ticket_id, new_status_name):
    ...
    try:
        ...
        # Запрос на данные о заявке и пользователе для уведомления
        cur.execute("""
        ...
        # Сообщение для пользователя с номером заявки и категорией
        if new_status_name == 'В работе':
            ...
            msg_user = f"❌ Приносим извинения, Ваша заявка №{ticket_id} ({category_name}) была отклонена. Можно попробовать подать новую заявку, если проблема ещё осталась!"
        ...
        # Проверка, остались ли еще активные заявки
        cur.execute("SELECT COUNT(*) FROM tickets WHERE status_id != %s AND status_id != %s", (3, 4))
        active_count = cur.fetchone()[0]
        conn.commit()
         
        write_msg(user_id, f"✅ Статус заявки №{ticket_id} успешно изменён на '{new_status_name}'")
        
        # Если заявок больше нет, возвращаем администратора в главное меню
        if active_count == 0:
            show_admin_menu(user_id)
        else:
            # Если заявки еще остались, обновить список
